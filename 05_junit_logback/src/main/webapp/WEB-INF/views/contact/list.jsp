...
<body>
          ...
          <tr>
            ...
            <td>
              <button type="button" class="btn-detail"
                data-contact-no="${contact.contactNo}">상세</button>
              <button type="button" class="btn-remove"
                data-contact-no="${contact.contactNo}">삭제</button>
            </td>
          </tr>
        </c:forEach>
      </tbody>
  ...
  $('.btn-remove').on('click', (evt)=>{
	  if(confirm('연락처를 삭제할까요?')){
	    const contactNo = $(evt.target).prev().data('contactNo'); // jQuery 방식으로 
	    location.href = '${contextPath}/contact/delete.do?contact-no=' + contactNo;
	  }
  });

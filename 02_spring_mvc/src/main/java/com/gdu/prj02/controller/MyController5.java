...
public class MyController5 {
  ...
  @GetMapping("/faq/add.do") // (1) redirect 이동
  public String add(RedirectAttributes redirectAttributes) { 
    
    // add 결과 
    int addResult = Math.random() < 0.5 ? 1 : 0; // 랜덤으로 이동 성공 
    ...
    redirectAttributes.addFlashAttribute("addResult", addResult); // addAttribute 는 전달 자체가 안된다!!!
    // addAttribute -> Model 을 쓰는 것과 같다
    
    // add 결과에 따른 이동
    String path = addResult == 1 ? "/faq/list.do" : "/main.do"; // (2) redirect 이동 
  ...
  public void modify(HttpServletRequest request, HttpServletResponse response) { // 반환타입 void
    // HttpServletRequest request -> ContextPath 를 가져오기 위해
